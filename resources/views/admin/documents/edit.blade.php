    ...
    <div class="container">
        ...
        <div>
            <div>
                ...
                <form action="{{ route('legaldocuments.update', $document->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <!-- BEGIN: input tex: title -->
                    <div class="intro-y col-span-12 sm:col-span-6">
                        <label>Cím</label>
                        <input type="text" class="input w-full border mt-2" placeholder="Cím" name="title"
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <select class="input w-full border mt-2" name="is_active" style="width: 120px;">
                            <option value="1" {{ $document->is_active == 1 ? 'selected' : '' }}>Aktív</option>
                            <option value="0" {{ $document->is_active == 0 ? 'selected' : '' }}>Inaktív</option>
                        </select>
                        @error('status')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <input type="number" class="input w-full border mt-2" placeholder="Sorrend" name="order"
                            style="width: 120px;" value="{{ $document->order }}" required>
                        @error('order')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <textarea id="body" name="body" class="hidden">{{ $document->body }}</textarea>
                    <div class="intro-y col-span-12">
                        <label>Leírás</label>
                        <div class="editor" id="editor">{!! $document->body !!}</div>
                        @error('body')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="intro-y col-span-12 flex items-center justify-center sm:justify-start mt-5">
                        <button type="submit"
                            class="btn btn-success flex items-center mr-3 cursor-pointer text-white">Frissítés</button>
                    </div>
