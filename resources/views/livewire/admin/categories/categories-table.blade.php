<div>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5" x-data="{ isOpen: false }">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <button @click="$dispatch('open-category-modal-this'); {{-- setTimeout(() => { setfocus(); }, 50); --}}"
                class="btn btn-primary shadow-md mr-2">Új kategória
                felvitele</button>
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-nowrap">KATEGÓRIA NEVE</th>
                        <th class="whitespace-nowrap">TERMÉKCSOPORT</th>
                ...
                <tbody>
                    @foreach ($all_categories as $item)
                        <tr wire:key="item-{{ $item->id }}" class="intro-x">
                            ...
                            <td>
                                <a href="" class="font-medium whitespace-nowrap">{{ $item->category_name }}</a>
                            </td>
                            <td class="">
                                @if ($item->category_id == null)
                                    {{ \App\Models\Product::TYPES[$item->type] }}
                                @else
                                    {{ $item->type }} ->
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    ...
                                    <button class="btn btn-sm btn-success flex items-center mr-3 text-white"
                                        @click="$dispatch('open-attr-modal-edit', { id: {{ $item->id }}} ); setTimeout(() => { setfocus(); }, 50);">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        ...
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
            ...
            <div class="col text-right text-muted">
                {{ __('messages.search-results', [
                    'firstItem' => $all_categories->firstItem(),
                    'lastItem' => $all_categories->lastItem(),
                    'total' => $all_categories->total(),
                ]) }}
        ...
        <x-slot:body>
            <form wire:submit="save">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <select wire:model.live="maincategory" required>
                    ...
                    <label class="font-medium text-gray-800">Kategória neve*</label>
                    <input wire:model.live="category_name" type="text" autocomplete="off" autofocus
                        id="attr_name" class="w-full outline-none rounded bg-gray-100 p-2 mt-2 mb-3" required />
