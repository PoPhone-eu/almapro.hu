    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12 lg:col-span-6">
            <form wire:submit="save">
                @include('livewire.admin.banners.partials.add-banner-images')
                @include('livewire.admin.banners.partials.add-banner-data')
                <div class="bg-gray-200 px-4 py-3 text-right">
                    @if ($error_msg != null)
                        <div class="text-red-500">{{ $error_msg }}</div>
                    @endif
                    <button type="submit" {{ $submitable == false ? 'disabled' : '' }}
                        class="btn btn-sm btn-success py-2 px-4 mr-2 text-white">Mentés</button>
                </div>
            </form>
        </div>
        <div class="intro-y col-span-3">
            ...
            @if ($selected_user != null)
                <div class="relative">Céges felhasználó: {{ $selected_user->full_name }}
                    <div
                        ...
                        <svg wire:click="clearSelectedUser" xmlns="http://www.w3.org/2000/svg" width="24"
